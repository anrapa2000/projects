function extractLeetCodeProblems() {
    ...
    problemRows.forEach(row => {
        ...
        // Title and Link
        const titleAnchor = row.querySelector('div[role="cell"] a[href^="/problems/"]');
        if (titleAnchor) {
            problemData.title = titleAnchor.textContent.trim(); // Extract the problem's title
            problemData.link = `https://leetcode.com${titleAnchor.getAttribute('href')}`; // Extract the full URL
        }

        // Frequency (percentage calculation based on progress bar width)
        const frequencyBar = row.querySelector('div[role="cell"] div[data-state="closed"] div[style*="width"]');
        if (frequencyBar) {
